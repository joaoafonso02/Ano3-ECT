...
function load3DObjects(sceneGraph) {
    ...
    const cylinderGeometry2 = new THREE.CylinderGeometry(0, 0.2, 1.5, 25, 1);
    const cylinderMaterial2 = new THREE.MeshPhongMaterial({ color: 0x8950ff });
    const cylinderObject2 = new THREE.Mesh(cylinderGeometry2, cylinderMaterial2);
    sceneGraph.add(cylinderObject2);

    // Set position of the cylinder
    
    cylinderObject2.translateX(1.5).translateY(1).translateZ(-1.5);
    ...
    // more trees
    var tree_2 = createTree(2);

    tree_2.position.x = -60;
...
function createTree( Treenumber ) {
    ...
    const cylinderRadius = 5 + Treenumber*1/2;

    const cylinderHeight = 20 + Treenumber*3;
    ...
    const coneHeight = 30 + Treenumber * 6;
