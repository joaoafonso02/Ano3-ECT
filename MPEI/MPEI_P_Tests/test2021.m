...
fprintf("Exercicio 1:\n");
% 1 a) 
N = 10000; % 10000 simulações
x1 = randi([1,4], 1, N); % 1 a 4 (com todos os valores igualmente prováveis) 
x2 = randi([4,8], 1, N); % 4 a 8 (com todos os valores também igualmente prováveis)
X = x2 - x1;
...
fprintf("1. a) Gráfico que apresenta a massa de probabilidade da variável X tem o nome de Figure 1.\n");
...
variancia = sum(fX.*(X.^2)) - media^2;  % Calculo da variancia 
desvioPadrao = sqrt(variancia); % Calculo do Desvio Padrao

fprintf("1. b) Média: %f, Variância: %f, Desvio Padrão: %f\n", media, variancia, desvioPadrao);
...
% > 3 - {4,5,6}
% 3/5 = P( ser maior que 3, sabendo que o primeiro número é 2)
...
vi = [0 1 0 0]';
T5 = T^4 * vi
fprintf('2a) A probabilidade de estar no estado 4 passado 5 transições é de %f.n\n', T5(4));
