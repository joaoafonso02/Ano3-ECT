...
void init_pfifo(PriorityFIFO* pfifo)
{
   ...
   pthread_mutex_init(&pfifo->mutex, NULL); // new
   pthread_cond_init(&pfifo->notEmpty, NULL); // new
   pthread_cond_init(&pfifo->notFull, NULL); // new
}
...
{
   require (pfifo != NULL, "NULL pointer to FIFO");   // a false value indicates a program error
   require (id <= MAX_ID + 1, "invalid id");              // a false value indicates a program error
   ...
   pthread_mutex_lock(&pfifo->mutex); // new
   while( full_pfifo(pfifo) ) { // new
      pthread_cond_wait(&pfifo->notFull, &pfifo->mutex); // new
   }
   ...
   pthread_cond_broadcast(&pfifo->notEmpty); // new
   pthread_mutex_unlock(&pfifo->mutex); // new
}
...
uint32_t retrieve_pfifo(PriorityFIFO* pfifo)
{
   ...
   pthread_mutex_lock(&pfifo->mutex);// new
   while( empty_pfifo(pfifo) ) {// new
      pthread_cond_wait(&pfifo->notEmpty, &pfifo->mutex);// new
   }

    
   //check_valid_id(pfifo->array[pfifo->out].id); // new
   ...
   pthread_cond_broadcast(&pfifo->notFull); // Broadcast to wake up all threads waiting for the FIFO to be not empty // new
   pthread_mutex_unlock(&pfifo->mutex);   // Release exclusive access to the FIFO // new
...
void print_pfifo(PriorityFIFO* pfifo)
{
   ...
   pthread_mutex_lock(&pfifo->mutex);  // get exclusive access to the FIFO // new
